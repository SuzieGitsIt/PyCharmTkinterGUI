# File:     PyCharmTkinterGUI_2023-04-11
# Version:  0.0.01
# Author:   Susan Haynes
# Comments/Notes:
...
# Git Token:
# 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
...
from tkinter import simpledialog                # Python popup window for sample entry fields
...
style.map('T.TButton', background=[('active', 'pressed', 'white'), ('!active', 'white'),
                                   ('active', '!pressed', 'grey')])             # active, not active, not pressed
style.map('T.TButton', relief=[('pressed', 'sunken'), ('!pressed', 'raised')])  # pressed, not pressed
style.configure('T.TButton', font=('Helvetica', '10'))
style.map('B.TButton', background=[('active', 'pressed', 'white'), ('!active', 'white'),
                                   ('active', '!pressed', 'grey')])             # Press me Button always hot pink when pressed
...
style.map('P.TButton', background=[('active', 'pressed', '#FF69B4'), ('!active', 'white'),
                                   ('active', '!pressed', 'grey')])             # Press me Button always hot pink when pressed
...
bg_img = Image.open(r'\\RXS-FS-02\userdocs\shaynes\My Documents\R&D - Software\Python\PyCharmTkinterGUI_2023-04-11\LAL.png')
...
lbl_disp_samp = ttk.Label(GUI, text="Number of Samples and Dioptic Sizes:", style='W.TLabel').place(x=50, y=600)
...
lbl_out_sum = ttk.Label(GUI, text='', style='W.TLabel', width=4)
lbl_out_sum.place(x=300, y=600)
...
def fun_cred():
    ...
    ws.cell(column=1, row=new_line, value=entry_cred.get()[:3]) # Why does this not always get printed to the workbook???
    print("entry_cred: ", entry_cred.get()[:3])                                 # Print can be removed after developed.
...
def fun_WO():
    ...
    ws.cell(column=2, row=new_line, value=entry_WO.get()[:6])   #  Why does this not always get printed to the workbook???
    print("entry_wo: ", entry_WO.get()[:6])
...
entry_d10 = 'na'
def get_samp ():
    global entry_s1, entry_d1, entry_s2, entry_d2, entry_s3, entry_d3, entry_s4, entry_d4, entry_s5, entry_d5
    global entry_s6, entry_d6, entry_s7, entry_d7, entry_s8, entry_d8, entry_s9, entry_d9, entry_s10, entry_d10
    ################# START NEW POP UP WINDOW #############################
    os.system('python ReceiveAndPass_2023-04-11.py')
...
def fun_samp(entry_samp):
    ...
    print("Total # of samples outisde the loop: ")
    print("Total # of sets outisde the loop: ")                                           # Will need to get the number of sets somehow... maybe check for blank entries and sum the full ones?
    print("entry_s1-10: ", entry_s1, entry_s2, entry_s3, entry_s4, entry_s5, entry_s6, entry_s7, ntry_s8, entry_s9, entry_s10) # Print can be removed after developed.
    ws.cell(GUI, column=3, row=new_line).value = entry_s1  # Why does this not always get printed to the workbook???
    ws.cell(GUI, column=3, row=new_line).value = entry_s2  # Why does this not always get printed to the workbook???
    ws.cell(GUI, column=3, row=new_line).value = entry_s3  # Why does this not always get printed to the workbook???
    ws.cell(GUI, column=3, row=new_line).value = entry_s4  # Why does this not always get printed to the workbook???
    ws.cell(GUI, column=3, row=new_line).value = entry_s5  # Why does this not always get printed to the workbook???
    ws.cell(GUI, column=3, row=new_line).value = entry_s6  # Why does this not always get printed to the workbook???
    ws.cell(GUI, column=3, row=new_line).value = entry_s7  # Why does this not always get printed to the workbook???
    ws.cell(GUI, column=3, row=new_line).value = entry_s8  # Why does this not always get printed to the workbook???
    ws.cell(GUI, column=3, row=new_line).value = entry_s9  # Why does this not always get printed to the workbook???
    ws.cell(GUI, column=3, row=new_line).value = entry_s10  # Why does this not always get printed to the workbook???

    lbl_out_sum.configure(text=Sum)                                      # Display sample entry from user on Diotpics GUI


excel_meas = 'null'                                     # excel_meas is what gets written inside the excel workbook
file_meas = 'null'                                       # fil_meas is what gets used for the excel file name
...
def fun_save():
    ...
    if excel_pr == 'Calibration':
        filepath = r"\\RXS-FS-02\userdocs\shaynes\My Documents\R&D - Software\Python\VS-TkinterGUI_2023-04-07/" + excel_oct + ' (Lumedica ' + fold_oct + ')' + '/' + dt.datetime.now().strftime(
            "%m-%d-%y") + ' EQ ' + eq_num_oct + ' CALIBRATION'
